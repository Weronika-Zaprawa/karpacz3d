Q3D.gui = {
  ...
  parameters: {
    ...
    cmd: {
      // commands for touch screen devices
  ...
  init: function (setupDefaultItems, params) {
    ...
    if (setupDefaultItems === undefined || setupDefaultItems == true) {
      this.layersFolder = this.gui.addFolder("Warstwy");
  ...
  initLayersFolder: function (scene) {
    ...
    var opacityChanged = function (value) {
      mapLayers[this.object.i].opacity = value;
    };
    ...
    for (var layerId in mapLayers) {
      ...
      subfolder
        .add(parameters.lyr[layerId], "v")
        .name("Widoczny")
        .onChange(visibleChanged);
      subfolder
        .add(parameters.lyr[layerId], "o")
        .min(0)
        .max(1)
  ...
  initCustomPlaneFolder: function (zMin, zMax) {
    ...
    // Plane color
    this.customPlaneFolder
      .addColor(parameters.cp, "c")
      .name("Color")
  ...
  addCommandsFolder: function () {
    ...
    folder
      .add(this.parameters.cmd, "rot")
      .name("Rotate Animation")
      .onChange(Q3D.application.setRotateAnimationMode);
    folder
      .add(this.parameters.cmd, "wf")
      .name("Wireframe Mode")
      .onChange(Q3D.application.setWireframeMode);
  },

  addHelpButton: function () {
    this.gui.add(this.parameters, "i").name("Skróty Klawiszowe");
  },
};
